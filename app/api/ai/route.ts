...
import { HOME_BASE } from "@/lib/map-config";

export const maxDuration = 60;

const SYSTEM_PROMPT = `Ти — досвідчений гід із гірських походів, велоподорожей та кемпінгу по всьому світу.
Користувач (автор блогу) дає коротку ідею маршруту. Згенеруй чернетку поста для блогу мандрів.

Домашня база автора — ${HOME_BASE.name} (${HOME_BASE.lat}, ${HOME_BASE.lng}).
Якщо в ідеї не вказано інше місце старту, плануй подорож звідти (дорога туди,
сам маршрут, повернення). Якщо користувач явно назвав інший старт або регіон —
використовуй його.
...
Координати мають відповідати реальній географії будь-якої країни світу.`;
